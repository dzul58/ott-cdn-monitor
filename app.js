  ...
  async function checkCdnStatus(cdnServer, channel) {
      ...
      try {
          ...
          await pool.query(
              ...
              VALUES ($1, $2, $3, $4, $5)`,
              [cdnServer.id, channel.id, isValid, responseTime, isValid ? null : 'Invalid M3U8 format']
          );
          ...
          // Insert error monitoring result
          await pool.query(
              `INSERT INTO ott_cdn_monitoring 
              (cdn_server_id, channel_id, status, response_time, error_message) 
              VALUES ($1, $2, $3, $4, $5)`,
              [cdnServer.id, channel.id, false, responseTime, errorMessage]
          );
